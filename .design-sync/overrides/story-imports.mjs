  ...
  const policyRedirect = {
    ...
    setup(b) {
      b.onResolve({ filter: /.*/ }, async (a) => {
        ...
        // never a dependency's internal sub-imports) - skip the extra async
        // b.resolve() round-trip below. Without this, a barrel package that
        // re-exports thousands of leaves (@mui/icons-material: ~2100 icons,
        // each individually named-imported by src/stories/components/DataTable.stories.tsx)
        // makes every one of those re-exports pay a second IPC round-trip to
        // the esbuild service; observed cost was minutes-to-unbounded for
