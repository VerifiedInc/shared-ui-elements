...
export function DataTableFieldFilterPanel({
  ...
  const renderSelect = (field: DataTableFilterField): JSX.Element => {
    ...
    return (
      <Autocomplete
        ...
        value={selected}
        onChange={(_, next) =>
          setValue(field, { kind: 'select', value: next?.value ?? null })
        }
  ...
  const renderMultiSelect = (
    ...
    <Autocomplete
      ...
      renderInput={(params) => (
        <TextField {...params} label={label} placeholder={field.placeholder} />
      )}
  ...
  const renderBoolean = (field: DataTableFilterField): JSX.Element => {
    ...
    const asString = current === null ? '' : current ? 'true' : 'false';
  ...
  return (
    <Popover
      ...
      onClose={onClose}
    >
      <Box sx={{ p: 1.5, minWidth: 360, maxWidth: 520 }}>
        {fields.length === 0 ? (
